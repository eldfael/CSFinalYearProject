...
public class WeaponBow : MonoBehaviour, Weapon
{
    float attackTimer;
    ...
    int damage = 1;
    int staminaCost = 1;
    Vector2 hitbox = new Vector2 (0.5f,0.25f);
    ...
    public void FixedUpdate()
    {
        if (transform.parent != null && transform.parent.gameObject == playerObject)
        {
            ...
            attackInterval = 5f / (float)playerObject.GetComponent<PlayerController>().GetAGI();
    ...
    public void OnAttack()
    {
        ...
        projectileController.Create(
            ...
            new Vector2(playerObject.transform.position.x, playerObject.transform.position.y) + (mousePosition - new Vector2(playerObject.transform.position.x, playerObject.transform.position.y)).normalized / 2, // Position
